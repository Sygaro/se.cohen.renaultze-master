...
class RenaultApi {
    ...
    encrypt(text) {
        if (!this.encryptionKey || this.encryptionKey === "") {
            console.warn('Encryption key not found - storing credentials as plain text (NOT RECOMMENDED for production)');
            return text;
        }
        try {
    ...
    decrypt(encryptedJson) {
        // If no encryption key is set, return the data as-is (assuming it's plain text)
        if (!this.encryptionKey || this.encryptionKey === "") {
            console.log('No encryption key available, treating data as plain text');
            // If it's a string, return it; if it's an object without iv, it might be plain text
            if (typeof encryptedJson === 'string') {
                return encryptedJson;
            }
            // If it's an object but has no iv property, it's likely plain text stored as string
            if (!encryptedJson || typeof encryptedJson !== 'object' || !encryptedJson.iv) {
                return encryptedJson;
            }
            // If we get here, we have encrypted data but no key - this is an error state
            console.error('Encrypted data found but no encryption key available');
        ...
        if (!encryptedJson || !encryptedJson.iv) {
            return encryptedJson;
        }
    ...
    async signUp() {
        ...
        */
        ...
        try {
            const loginRes = await this._gigyaLogin();
            const login_token = loginRes.login_token;
            console.log('login_token ' + login_token);
            
            const accountRes = await this._gigyaGetAccountInfo(login_token);
            const person_id = accountRes.personId;
            console.log('personId ' + person_id);
            
            const jwtRes = await this._gigyaGetJWT(login_token);
            const id_token = jwtRes.id_token;
            console.log('id_token ' + id_token);
            
            return await this._kamereonGetPerson(id_token, person_id);
        } catch (error) {
    ...
    async getDevices() {
        ...
        */
        ...
        try {
            const id_token = await this._getIdToken();
            return await this._kamereonGetVehicles(id_token);
        } catch (error) {
    ...
    async getBatteryStatus() {
        ...
        */
        ...
        try {
            const id_token = await this._getIdToken();
            return await this._kamereonGet(id_token, "battery-status", 2);
        } catch (error) {
    ...
    async getCockpit() {
        ...
        */
        ...
        console.log('getCockpit() start');

        if (this.supportCockpit() === false) {
            console.log('getCockpit() - not supported');
            return ({ status: "notSupported", data: null });
        }
        
        try {
            const id_token = await this._getIdToken();
            return await this._kamereonGet(id_token, "cockpit", 1);
        } catch (error) {
            console.error('getCockpit failed:', error);
            throw error;
        }
    }
    ...
    async getACStatus() {
        ...
        */
        ...
        try {
            const id_token = await this._getIdToken();
            return await this._kamereonGet(id_token, "hvac-status", 1);
        } catch (error) {
            console.error('getACStatus failed:', error);
            throw error;
        }
    }
    ...
    async startAC(temp) {
        ...
        try {
            const id_token = await this._getIdToken();
            return await this._kamereonPost(id_token, "actions/hvac-start", body, 1);
        } catch (error) {
            console.error('startAC failed:', error);
            throw error;
        }
    }
    ...
    async stopAC() {
        ...
        try {
            ...
            console.error('stopAC failed:', error);
            throw error;
        }
    }
    ...
    async getLocation() {
        ...
        */
        ...
        try {
            const id_token = await this._getIdToken();
            return await this._kamereonGet(id_token, "location", 1);
        } catch (error) {
    ...
    async chargingStart() {
        ...
        try {
            const id_token = await this._getIdToken();
            return await this._kamereonPostCharge(id_token, "charge/pause-resume", body, 1);
        } catch (error) {
            console.error('chargingStart failed:', error);
            throw error;
        }
    }
    ...
    async chargingStop() {
        ...
        try {
            ...
            console.error('chargingStop failed:', error);
            throw error;
        }
    }
    ...
    async getChargeMode() {
        ...
        */
        ...
        try {
            const id_token = await this._getIdToken();
            // Try the newer charging-settings endpoint first
            try {
                const result = await this._kamereonGet(id_token, "charging-settings", 1);
                console.log('Using charging-settings endpoint');
                // Transform the response to match charge-mode format
                if (result.status === 'ok' && result.data.data.attributes) {
                    const mode = result.data.data.attributes.mode;
                    result.data.data.attributes.chargeMode = mode === 'scheduled' ? 'scheduled' : 'always';
                }
                return result;
            } catch (settingsError) {
                console.log('charging-settings failed, falling back to charge-mode');
                // Fall back to original charge-mode endpoint
                return await this._kamereonGet(id_token, "charge-mode", 1);
            }
        } catch (error) {
            console.error('getChargeMode failed:', error);
            throw error;
        }
    }
    ...
    async setChargeMode(mode) {
        ...
        try {
            const id_token = await this._getIdToken();
            return await this._kamereonPost(id_token, "actions/charge-mode", body, 1);
        } catch (error) {
            console.error('setChargeMode failed:', error);
            throw error;
        }
    }

    async reAuthenticate(username, password) {
        // Public method to allow re-authentication with new credentials
        console.log('reAuthenticate() start');
        
        // Update credentials
        this.settings.username = this.encrypt(username);
        this.settings.password = this.encrypt(password);
        ...
        try {
            ...
            return { status: 'ok', message: 'Re-authentication successful', data: result.data };
        } catch (error) {
    ...
    async _getIdToken() {
        ...
        try {
            const loginRes = await this._gigyaLogin();
            const jwtRes = await this._gigyaGetJWT(loginRes.login_token);
            
            // Cache the token with expiry (JWT expiration is 900 seconds, cache for 800 to be safe)
            this.cachedToken = jwtRes.id_token;
            this.tokenExpiry = Date.now() + (800 * 1000);
            
            return jwtRes.id_token;
        } catch (error) {
    ...
    async _refreshToken() {
        // Force a token refresh by clearing the cache
        this.cachedToken = null;
        this.tokenExpiry = null;
    ...
    async _kamereonGet(id_token, path, version) {
        ...
        try {
            ...
            console.error('_kamereonGet error:', err.response?.status, err.message);
            ...
            if (err.response?.status === 401 || err.response?.status === 403) {
                ...
                try {
                    ...
                    const retryResponse = await axios.get(
                        this.configuration.kamereon_url + '/commerce/v1/accounts/' + this.settings.accountId + "/kamereon/kca/car-adapter/v" + version + "/cars/" + this.settings.vin + "/" + path + "?country=" + this.settings.country, {
                        headers: {
                            'x-gigya-id_token': newToken,
                            'apikey': this.configuration.kamereon_api_key
                        }
                    });
                    return ({ status: "ok", data: retryResponse.data });
                } catch (retryErr) {
    ...
    async _kamereonPost(id_token, path, data, version) {
        ...
        try {
            ...
            console.error('_kamereonPost error:', err.response?.status, err.message);
    ...
    async _kamereonPostCharge(id_token, path, data, version) {
        ...
        try {
            ...
            console.error('_kamereonPostCharge error:', err.response?.status, err.message);
